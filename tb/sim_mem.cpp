...
void sim_mem_write(Vtest_top_rom* rom,uint32_t addr, size_t length, const void* bytes)
{
...
void sim_mem_load_bin(Vtest_top_rom* rom, std::string fn)
{
...
// void sim_mem_write(Vinst_fetch_rom* rom,uint32_t addr, size_t length, const void* bytes)
// {
//     //out of boundary protection?
//     for (int i = 0 ; i < length ; i +=4 ) {
//         rom->writeByte(addr+i,*((unsigned char*)bytes+i+3));
...
// void sim_mem_load_bin(Vinst_fetch_rom* rom, std::string fn)
// {
//     std::ifstream bpfs(fn, std::ios::binary|std::ios::ate);
//     std::ifstream::pos_type pos = bpfs.tellg();
//     int f_length = pos;
